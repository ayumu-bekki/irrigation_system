...
uint32_t GetAdcVoltage(const int32_t adcChannelNo, const int32_t round) {
  ...
  adc_oneshot_config_channel(
      adcHandle, static_cast<adc_channel_t>(adcChannelNo), &adcConfig);
  ...
  for (int32_t i = 0; i < round; ++i) {
    int32_t adcValue = 0;
    adc_oneshot_read(adcHandle, static_cast<adc_channel_t>(adcChannelNo),
                     reinterpret_cast<int*>(&adcValue));
    int32_t adcVoltage = 0;
    adc_cali_raw_to_voltage(adcCaliHandle, adcValue,
