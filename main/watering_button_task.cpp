...
WateringButtonTask::WateringButtonTask(
  ...
  io_conf.pull_up_en =
      GPIO_PULLUP_DISABLE;  // IO34 don’t have internal pull-down resistors
  ...
  gpio_isr_handler_add(static_cast<gpio_num_t>(CONFIG_WATERING_INPUT_GPIO_NO),
                       this->GpioIsrHandler, this);
}

WateringButtonTask::~WateringButtonTask() {
  gpio_isr_handler_remove(
      static_cast<gpio_num_t>(CONFIG_WATERING_INPUT_GPIO_NO));
}
...
void IRAM_ATTR WateringButtonTask::GpioIsrHandler(void *pData) {
  ...
  WateringButtonTask *const pWateringButtonTask =
      static_cast<WateringButtonTask *>(pData);
